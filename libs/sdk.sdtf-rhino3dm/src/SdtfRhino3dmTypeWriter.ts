...
export class SdtfRhino3dmTypeWriter implements ISdtfTypeWriter {
    ...
    writeComponent(component: ISdtfWriteableAttribute | ISdtfWriteableDataItem): void {
        ...
        if (
            !this.validator.validateExternalRepresentationOfComponent(
                typeHint,
                component.value,
                component.accessor
            )
            ...
            throw new SdtfError(
                `Cannot write component of type '${typeHint}': Specified component data is invalid (for rhino3dm objects make sure that '.isValid' is truthy).`
            );
        }
    ...
    postProcessComponents(components: (ISdtfWriteableAttribute | ISdtfWriteableDataItem)[]): void {
        const rhinoComponents = components.filter((c) =>
            enumValues(SdtfRhinoTypeHintName).includes(c.typeHint?.name ?? '')
        );
        ...
        rhinoComponents.forEach((component) => {
            ...
            if (
                !this.validator.validateInternalRepresentationOfComponent(
                    component.typeHint!.name!,
                    component.value,
                    component.accessor
                )
            ) {
                throw new SdtfError(
                    `Cannot write component of type '${component.typeHint!.name}': Invalid component.`
                );
            }
        });
    }
