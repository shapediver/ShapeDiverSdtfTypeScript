...
describe('writeComponent', function () {
    ...
    beforeAll(() => {
        origValidateExternalRepresentationOfComponent =
            SdtfRhino3dmTypeValidator.prototype.validateExternalRepresentationOfComponent;
    });
    ...
    test('invalid grasshopper component; should throw', () => {
        // @ts-ignore: Mock
        SdtfRhino3dmTypeValidator.prototype.validateExternalRepresentationOfComponent = jest.fn(
            () => false
        );

        const component = factory.createDataItem(
            'value',
...
describe('postProcessComponents', function () {
    ...
    test('no rhino components; should return', () => {
        const components = [factory.createDataItem('value', 'string')];
    ...
    test.each([
        ...
        [
            ...
            () => rhino.NurbsSurface.create(0, false, 1, 2, 3, 4),
        ],
        ...
        'single component of type %s; should store rhino component in single buffer',
        (type, createComponent) => {
            ...
            expect(components[0].accessor!.bufferView!.contentType).toBe(
                writer.CONTENT_TYPE_RHINO3DM
            );
        }
    );
    ...
    test('mixed components; should store rhino components in single buffer and link them', () => {
        ...
        expect(
            components[1].accessor!.bufferView === components[2].accessor!.bufferView
        ).toBeTruthy();
    });
});
