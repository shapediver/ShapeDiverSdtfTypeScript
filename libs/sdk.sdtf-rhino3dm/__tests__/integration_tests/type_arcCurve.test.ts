...
describe("type arc-curve", function () {
    ...
    test("create sdTF, read and extract content; should return valid rhino component instance", async () => {
        ...
        const content = new rhino.ArcCurve()
        expect(content.isValid).toBeTruthy()

        const writeableAsset = constructor.getWriter().createSimpleDataSdtf("", [ {
            content,
            typeHint: SdDtfRhinoTypeHintName.RHINO_ARC_CURVE,
        } ])
        ...
        SdDtfRhino3dmTypeGuard.assertArcCurve(contentInstance)
    })
