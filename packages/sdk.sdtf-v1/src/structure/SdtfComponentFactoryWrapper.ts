...
export class SdtfComponentFactoryWrapper implements SdtfComponentFactoryWrapper {
    ...
    buildFromObject<T> (
        ...
        if (!isDataObject(componentDataObject)) throw new SdtfError(`Invalid item at ${ propertyName }: Item must be an object.`)
    ...
    buildFromArray<T> (
        ...
        if (!Array.isArray(componentDataArray)) throw new SdtfError(`Invalid content property: '${ propertyName }' must be an array.`)

        return componentDataArray.map((componentDataItem: unknown, i) => {
            if (!isDataObject(componentDataItem)) throw new SdtfError(`Invalid item at ${ propertyName }[${ i }]: Item must be an object.`)
    ...
    mapHierarchyRepresentation (target: ISdtfPartialComponentList, src: ISdtfWriteableComponentList | ISdtfReadableComponentList): void {
        ...
        target.attributes.forEach((attributes, index) => {
            ...
            Object.entries(attributes.entries).forEach(([ name, attribute ]) => {
                const srcComponent: ISdtfWriteableAttribute | ISdtfReadableContentComponent = src.attributes[index].entries[name]
