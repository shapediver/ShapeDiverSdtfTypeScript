import { ISdtfWriteableComponentFactory } from '@shapediver/sdk.sdtf-core';
import { create } from '../../../../src';
import { SdtfGrasshopperSdtfBuilder } from '../../../../src/writer/builder/SdtfGrasshopperSdtfBuilder';
...
describe('addChunkForListData', function () {
    ...
    test('list given; should create a chunk with a single node that holds all data', () => {
        ...
        const builder = new SdtfGrasshopperSdtfBuilder(factory);
...
describe('addChunkForTreeData', function () {
    ...
    test('invalid tree - branches and paths are not equal; should throw', () => {
        ...
        const builder = new SdtfGrasshopperSdtfBuilder(factory);
        expect(() => builder.addChunkForTreeData('', { branches, paths })).toThrow(
            /'branches' and 'paths' of the grasshopper structure must have the same number of elements/
        );
    });
    ...
    test('invalid tree - path holds non-integers; should throw', () => {
        ...
        const builder = new SdtfGrasshopperSdtfBuilder(factory);
        expect(() => builder.addChunkForTreeData('', { branches, paths })).toThrow(
            /'paths' of the grasshopper structure must only consist of integer values/
        );
    });
    ...
    test('invalid tree - data items without type hint; should throw', () => {
        ...
        const builder = new SdtfGrasshopperSdtfBuilder(factory);
        expect(() => builder.addChunkForTreeData('', { branches, paths })).toThrow(
            /All data items in 'tree.branches' must contain a type hint/
        );
    });

    test('invalid tree - data items with different type hints; should throw', () => {
        const branches = [
            [factory.createDataItem('foo', 'bar')],
            [factory.createDataItem('foo', 'baz')],
        ];
        const paths = [[0], [1]];

        const builder = new SdtfGrasshopperSdtfBuilder(factory);
        expect(() => builder.addChunkForTreeData('', { branches, paths })).toThrow(
            /All data items in 'tree.branches' must have the same type hint/
        );
    });

    test('tree given; should create a chunk with multiple nodes which hold the data', () => {
        const branches = [
            [factory.createDataItem('foo', 'string'), factory.createDataItem('bar', 'string')],
        ...
        builder.addChunkForTreeData(
            'ce0065af-8a90-4cd7-aa83-b8551fa7174a',
            { branches, paths },
            chunkAttributes
        );
        const grasshopperAsset = builder.build();
