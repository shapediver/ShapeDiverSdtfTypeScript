...
describe('createSimpleDataSdtf', function () {
    test('should create a single chunk that holds all the data', () => {
        ...
        expect(
            writeableAsset.chunks[0].items[0].attributes!.entries['random'].value
        ).toBeGreaterThanOrEqual(0);
        expect(
            writeableAsset.chunks[0].items[0].attributes!.entries['random'].typeHint
        ).toBeDefined();
        expect(
            writeableAsset.chunks[0].items[0].attributes!.entries['random'].typeHint!.name
        ).toBe('double');
        ...
        expect(writeableAsset.chunks[0].items[1].accessor!.bufferView!.buffer!.data).toStrictEqual(
            new ArrayBuffer(123)
        );
        ...
        expect(
            writeableAsset.chunks[0].items[2].attributes!.entries['random'].value
        ).toBeGreaterThanOrEqual(0);
        expect(
            writeableAsset.chunks[0].items[2].attributes!.entries['random'].typeHint
        ).toBeDefined();
        expect(
            writeableAsset.chunks[0].items[2].attributes!.entries['random'].typeHint!.name
        ).toBe('double');
    });
});
