...
describe('removeDuplicatedTypeHints', function () {
    test('one unique type hint; should merge unique type hints and set references', () => {
        ...
        const componentList: ISdtfWriteableComponentList = {
            ...
            nodes: [node],
            typeHints: [
                ...Object.values(attributes.entries).map((a) => a.typeHint!),
                chunk.typeHint!,
                dataItem.typeHint!,
                node.typeHint!,
            ],
...
describe('resolveBuffers', function () {
    ...
    test('internal and external buffers; should merge buffers depending on their uri property and set references', () => {
        ...
        const internalBuffer1 = factory.createBufferView({
            data: new ArrayBuffer(30),
            contentType: 'a',
        });
        const internalBuffer2 = factory.createBufferView({
        ...
        const componentList: ISdtfWriteableComponentList = {
            ...
            attributes: [],
            buffers: [
                internalBuffer1.buffer!,
                internalBuffer2.buffer!,
                externalBufferView1.buffer!,
                externalBufferView2.buffer!,
                externalBufferView3.buffer!,
            ],
            bufferViews: [
                internalBuffer1,
                internalBuffer2,
...
/* Helper functions */
function createComponentListWithNodesAndItems(
...
function createComponentListWithChunksAndNodes(
